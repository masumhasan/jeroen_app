...
const EditRecipe = () => {
  ...
  useEffect(() => {
    if (recipeId) loadRecipe(recipeId as string);
  }, [recipeId]);

  const loadRecipe = async (id: string) => {
    try {
      const recipe = await recipeService.getUserRecipe(id);
      setName(recipe.name || "");
      const cats = Array.isArray(recipe.category) ? recipe.category : [recipe.category || "Ontbijt"];
      setCategories(cats);
      setPersonsServing(String(recipe.personsServing || 1));
      setCookingTip(recipe.cookingTip || "");
      setIngredients(recipe.ingredients?.length > 0 ? recipe.ingredients : [""]);
      setSteps(recipe.recipeDetails?.length > 0 ? recipe.recipeDetails : [""]);
      setMacros({
        kcal: String(recipe.nutrition?.kcal ?? "0"),
        khd: String(recipe.nutrition?.khd ?? "0"),
        vetten: String(recipe.nutrition?.vetten ?? "0"),
        eiwitten: String(recipe.nutrition?.eiwitten ?? "0"),
        vezels: String(recipe.nutrition?.vezels ?? "0"),
      });
      if (recipe.recipeImage) {
        const img = recipe.recipeImage.startsWith("http")
          ? recipe.recipeImage
          : `http://10.0.2.2:5000${recipe.recipeImage}`;
