...
export default function BookRecipes() {
  ...
  useFocusEffect(
    useCallback(() => {
      if (bookSku) fetchRecipes(bookSku);
    }, [bookSku])
  );

  const fetchRecipes = async (sku: string) => {
    setLoading(true);
    try {
      const data = await recipeService.getRecipesByBookSku(sku);
      setRecipes(data);
    } catch (error) {
      console.error("Error fetching book recipes:", error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <SafeAreaView className="flex-1 bg-white" edges={["top"]}>
      <View className="flex-1 px-5">
        ...
        <View className="flex-row items-center justify-center mb-5 relative">
          ...
          <Text
            className="text-[18px] font-bold text-[#111] text-center"
            numberOfLines={1}
            style={{ maxWidth: "75%" }}
          >
            {bookTitle ?? "Recepten"}
        ...
        {loading ? (
          ...
          <View className="flex-1 items-center justify-center px-6">
            <View className="w-16 h-16 rounded-full bg-[#F0F4EE] items-center justify-center mb-4">
              <Ionicons name="book-outline" size={32} color="#7C8B74" />
            </View>
            <Text className="text-[17px] font-semibold text-[#222] text-center">
              Nog geen recepten
            </Text>
            <Text className="text-[14px] text-[#8E8E93] text-center mt-2">
              Er zijn nog geen recepten beschikbaar voor dit boek.
            </Text>
          </View>
        ) : (
          <>
            <Text className="text-[13px] text-[#8E8E93] mb-4">
